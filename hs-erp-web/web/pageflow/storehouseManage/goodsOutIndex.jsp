...
<html>
	<head>
		...
		<script type="text/javascript"><!--
			...
			function queryGoods(tagname){
				dojo.xhrPost({
	           	...
	           	load: function (data,ioArgs){
		            ...
		            queryGoodsGrid.setModel(store);
		            queryGoodsGrid.refresh();
				},
	           	error: callError
	        	});
			}
			...
			dojo.xhrPost({
	           	...
	           	load: function (data,ioArgs){
					...
					data = eval(data);
					var store = new dojox.grid.data.Objects(null, data);
		            outStoreGrid.setModel(store);
		            outStoreGrid.refresh();
				},
	           	error: callError
	        	});
			}
		...
		var structure = [{"cells": [[
             {"field": "no", "name": "出库编号","width": "100px"},
			...
			//确认入库表结构
			var outStorehouseStructure = [{"cells": [[
             {"field": "no", "name": "出库编号","width": "100px"},
             {"field": "storehouse", "name": "仓库"},            
             {"field": "place", "name": "库位"},
             {"field": "number", "name": "数量"},
		   ...
		   //入库详情表结构
		   var outStoreDetailStructure = [{"cells": [[
             {"field": "no", "name": "出库编号","width": "100px"},
             {"field": "inStoreDate", "name": "出库日期"}, 
             {"field": "adminName", "name": "出库人"},            
		...
		dojo.addOnLoad(function(){
             queryGoodsGrid.setStructure(structure);
             var mainTabContainer = dijit.byId("mainTabContainer");
             var inStoreDetail=dijit.byId("outStoreDetail");//
		...
		<div id="mainTabContainer" dojoType="dijit.layout.TabContainer"
			...
			<div dojoType="dijit.layout.ContentPane" title="查询物品" id="queryTab">
				<form action="" id="queryForm" name="queryForm">
					...
					<div dojoType="dojox.Grid" id="queryGoodsGrid" jsId="queryGoodsGrid"
						style="height: 90%"></div>
				</form>
				<br />
			...
			<div dojoType="dijit.layout.ContentPane" title="物品出库" id="import">
				出库货号：
				...
				<div dojoType="dojox.Grid" id="outStoreGrid" jsId="outStoreGrid"
					style="height: 90%"></div>
				</div>
				
				<div dojoType="dijit.layout.ContentPane" title="出库详情" id="outStoreDetail">
				出库货号：
				...
				<div dojoType="dojox.Grid" id="outStoreDetailGrid" jsId="outStoreDetailGrid"
					style="height: 90%"></div>
				</div>	
				<script type="text/javascript">
					function showGrid(){
					    var outStoreGrid = dijit.byId("outStoreGrid");
						outStoreGrid.setStructure(outStorehouseStructure);
						outStoreGrid.refresh();
					}
					
					function outStoreDetailGrid(){//
						var outStoreDetailGrid=dijit.byId("outStoreDetailGrid");
						outStoreDetailGrid.setStructure(outStoreDetailStructure);
						outStoreDetailGrid.refresh();
					}
